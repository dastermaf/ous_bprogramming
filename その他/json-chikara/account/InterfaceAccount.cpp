...
class InterfaceAccount {
    ...
    bool InterfaceAccount::input_search_info(const unsigned int column = 1) {
        string type;
        string prompt;
    ...
    int InterfaceAccount::register_account() {
        ...
        data.nickname = user_input.c_str();
        clear();

        while (true) {
            cout << "Set up balance: " << out;
            cin >> user_input;

            if (!indigit(user_input)) {
